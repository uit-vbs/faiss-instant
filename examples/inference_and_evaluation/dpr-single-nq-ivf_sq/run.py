...
parser.add_argument(
    "--title",
    action="store_true",
    help="Use title or not for matching answers. By default, it is False, which is the same setting of the official DPR code.",
)
parser.add_argument(
    "--official_tokenization",
    action="store_true",
    help="Whether to use the official tokenization (actually a wrongly written one). By default, it is False, whichi would give a little higher performance.",
)
parser.add_argument(
    "--device",
    type=str,
    choices=["cpu", "cuda"],
    default="cpu",
    help="Which device to use for query encoding",
)
...
with open("downloads/data/retriever/qas/nq-test.csv", "r") as f:
    ...
    for qid, row in enumerate(reader):
        qid = str(qid)
        nq_test[qid] = {"question": row[0], "answers": eval(row[1])}
...
def query(question):
    ...
    rels = [
        dict(sorted(rels_.items(), key=lambda x: x[1], reverse=True)) for rels_ in rels
    ]
    return rels
