...
class FaissIndex(object):
    ...
    def load(self, index_name=None, use_gpu=False):
        ...
        assert len(
            index_list
        ), f"No valid index! Please make sure there is at least one pair called 'xxx{self.ids_suffix}' and 'xxx{self.index_suffix}' under the resource path."
        if index_name is None:
            index_name = index_list[0]
        assert (
            index_name in index_list
        ), f"Index '{index_name}' does not exist! Possible ones: {index_list}"

        ids_path = os.path.join(self.resources_path, f"{index_name}{self.ids_suffix}")
        index_path = os.path.join(
            self.resources_path, f"{index_name}{self.index_suffix}"
        )
        self.logger.info(f"Loading index {index_name}")
        self.index = faiss.read_index(index_path)
        assert self.index.is_trained, "The index has not been trained!"
        if use_gpu:
            assert type(self.index) in [
                faiss.IndexFlat,
                faiss.IndexIVFFlat,
                faiss.IndexIVFPQ,
                faiss.IndexIVFScalarQuantizer,
            ], "The index type is not support for GPU by Faiss! More details: https://github.com/facebookresearch/faiss/wiki/Faiss-on-the-GPU#implemented-indexes."
            if type(self.index) == faiss.IndexIVFPQ:
                assert self.index.pq.code_size in [
                    1,
                    2,
                    3,
                    4,
                    ...
                    64,
                    96,
                ], "For IVFPQ with GPU, the code size is not supported by Faiss. More details: https://github.com/facebookresearch/faiss/wiki/Faiss-on-the-GPU#implemented-indexes."
    ...
    def reconstruct(self, _id):
        ...
        index = self.index
        try:
            index = faiss.index_gpu_to_cpu(
                index
            )  # GPU index does not support this method
        except:
            pass
        try:
    ...
    def explain(self, qvec, _id):
        ...
        assert metric in [
            ...
            faiss.METRIC_L2,
        ], "This method only supports IP or L2 metrics."
